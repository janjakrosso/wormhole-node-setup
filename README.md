...
```
service file creating
```console
nano name.sh
```
We clean the file, write the following codes and exit.
```console
#!/bin/bash
#check docker cmd
...
```
lets run our service file and check
```console
bash ./name.sh

curl -X POST -H "Content-Type:application/json" --data '{"jsonrpc":"2.0","method":"eth_version","id":64}' http://127.0.0.1:8545
```
Lets run the following command to check.
```console
nano monitor.sh
```
We clean the file, write the following codes and exit.
...
main "$@"
```
Lets run the service file again.
```console
bash ./monitor.sh
```
thats all.
